...
#'
#' The original MNIST handwritten digit is the image of 28*28 = 784 pixels. 
#' The pre-processing procedure is as follows.
#' First, the original 28 by 28 image is separated into 14 by 14 clusters, each one is a 2 by 2 block.
#' For each cluster, the maximal value in its cells is taken, and therefore, the original MNIST 
#' image is reduced into 14 by 14 image. Because the surrounding cells of the 
#' reduced image are usually zero value, we only take the center 
#' 10 by 10 sub-image by cutting the edge cells. 
